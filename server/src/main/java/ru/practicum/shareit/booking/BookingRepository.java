...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT b FROM Booking b JOIN b.item i ON b.item = i WHERE  i.owner.id = :userId AND b.start> :timeNow " +
            "ORDER BY b.start DESC")
    Page<Booking> findOwnerFuture(@Param("userId") long userId, Pageable pageable, @Param("timeNow") long timeNow);
    ...
    Page<Booking> findOwnerCurrent(@Param("userId") long userId, Pageable pageable, @Param("timeNow") long timeNow);

    @Query("SELECT b FROM Booking b JOIN b.item i ON b.item = i WHERE i.owner.id = :userId AND b.status = :status")
    Page<Booking> findByOwnerIdAndStatus(@Param("userId") long userId, Pageable pageable, @Param("status") BookingStatus status);

    @Query("SELECT b FROM Booking b JOIN b.item i ON b.item = i WHERE i.owner.id = :userId AND b.end< :timeNow")
    Page<Booking> findOwnerPast(@Param("userId") long userId, Pageable pageable, @Param("timeNow") long timeNow);
